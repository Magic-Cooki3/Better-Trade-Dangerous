...
class Station:
    ...
    __slots__ = (
        ...
        'odyssey', 'carrierDockingAccess', 'itemCount', 'dataAge',
    )
    ...
    def __init__(
            ...
            itemCount=0, dataAge=None, carrierDockingAccess: str = '?',
            ):
        ...
        # For Fleet Carriers, the docking access policy if known (e.g. 'All', 'Friends', 'Squadron', 'Squadron Friends', 'None', or '?')
        self.carrierDockingAccess = carrierDockingAccess or '?'
...
class TradeDB:
    ...
    def reloadCache(self):
        ...
        if self.dbPath.exists():
            ...
            if not changedPaths:
                ...
                # Ensure unknown stations in .prices do not abort the import.
                prev_ignore = getattr(self.tdenv, "ignoreUnknown", False)
                try:
                    ...
                    # Restore previous behavior to avoid side effects elsewhere.
                    self.tdenv.ignoreUnknown = prev_ignore
                return
        ...
        # When rebuilding the DB cache from sources, temporarily ignore unknown
        # entries to avoid aborting on stale/duplicate entries in prices data.
        prev_ignore = getattr(self.tdenv, "ignoreUnknown", False)
        try:
            self.tdenv.ignoreUnknown = True
    ...
    def _loadStations(self):
        ...
        CAUTION: Will orphan previously loaded objects.
        """
        # Attempt to load carrier docking access if present; fall back gracefully on older DBs.
        stationByID = {}
            ...
            with closing(self.query(stmt)) as cur:
                for (
                    ...
                    station = Station(
                        ...
                        0, None, carrierDockingAccess or '?',
                    )
    ...
    def addLocalStation(
        ...
        station = Station(
            ...
            carrierDockingAccess=carrierDockingAccess or '?',
        )
    ...
    def getDestinations(
        ...
        path_iter = iter(
          ...
          if (station.planetary == 'N' if noPlanet else True) and
            ...
            (station.lsFromStar > 0 and station.lsFromStar <= maxLsFromStar if maxLsFromStar else True) and
            # If destination is a Fleet Carrier, require explicit public access ('All').
            (False if (getattr(station, 'fleet', '?') == 'Y' and getattr(station, 'carrierDockingAccess', '?') != 'All') else True)
        )
