...
def processPrices(tdenv: TradeEnv, priceFile: Path, db: sqlite3.Connection, defaultZero: bool) -> tuple[ProcessedStationIds, ProcessedItems, ZeroItems, int, int, int, int]:
    ...
    def changeStation(matches: re.Match) -> None:
        ...
        if newID < 0:
            ...
            # Duplicate station header in the same prices file.
            # When ignoring unknowns, downgrade to a warning and treat the later
            # block as the authoritative one; otherwise raise.
            if not corrected:
                if not ignoreUnknown:
                    ...
                    if not quiet:
                        tdenv.NOTE(
                            "Duplicate station entry for {} at line {}; previous at line {} — using later block",
                            facility, lineNo, processedStations[newID]
                        )
                    # fall through to switch station context to this later block
