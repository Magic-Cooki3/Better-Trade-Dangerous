...
def checkStationSuitability(cmdenv, calc, station, src = None):
    ...
    if np and station.planetary != 'N':
        ...
        return False
    # Exclude Fleet Carriers unless docking access is explicitly 'All'
    try:
        if station.fleet == 'Y':
            access = getattr(station, 'carrierDockingAccess', '?')
            if access != 'All':
                if src and src != "--from":
                    raise CommandLineError(
                        "{} station {} is a carrier not open to all ({}).".format(
                            src, station.name(), access
                        )
                    )
                return False
    except AttributeError:
