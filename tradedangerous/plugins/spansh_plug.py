...
if dataclass:
    ...
    Station = namedtuple('Station',
                         ...
                         'market,black_market,shipyard,outfitting,rearm,refuel,repair,planetary,type,carrier_docking_access,modified')
...
class ImportPlugin(plugins.ImportPluginBase):
    ...
    def ensure_station(self, station: Station) -> None:
        ...
        # Attempt to include carrier docking access when available; fall back if older DBs lack the column
        try:
            self.execute(
                '''
                ...
                station.carrier_docking_access,
                station.modified,
                station.type,
                commitable=True,
            )
...
def ingest_stations(system_data):
    ...
    for target in targets:
        for station_data in target.get('stations', ()):
            ...
            carrier_docking_access = station_data.get('carrierDockingAccess')
            yield (
                Station(
                    ...
                    carrier_docking_access=carrier_docking_access,
